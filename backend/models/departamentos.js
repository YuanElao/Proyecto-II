...
class Departamento {
  ...
  static async listDep() {
    const result = await pool.query(
      "SELECT * FROM departamento ORDER BY d_name"
    );
  ...
  static async addDep(d_name) {
    const existe = await pool.query(
      "SELECT * FROM departamento WHERE d_name = $1",
      [d_name]
    );
  ...
  static async editDep(id_departamento, d_name) {
    ...
    await pool.query(
      "UPDATE departamento SET d_name = $1 WHERE id_departamento = $2",
      [d_name, id_departamento]
    );
  }
  ...
  static async deleteDep(id_departamento) {
    await pool.query(
      "UPDATE trabajadores SET id_departamento = NULL WHERE id_departamento = $1",
      [id_departamento]
    );
