...
async validationForDate(fecha) {
    ...
    if (asistencia > 0) return { valido: false, message: "Asistencia ya registrada en esta fecha"};
    if (permiso > 0) return { valido: false, message: "Existe un permiso activo para esta fecha"};
    if (falta > 0) return { valido: false, message: "Falta ya registrada en esta fecha"};
