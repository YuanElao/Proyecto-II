...
class Trabajador {
  ...
  async registerWorker() {
    //Formatear los datos
    const nombre =
      this.tname.charAt(0).toUpperCase() + this.tname.slice(1).toLowerCase();
    const apellido =
      this.tapellido.charAt(0).toUpperCase() +
      this.tapellido.slice(1).toLowerCase();
    ...
    do {
      ...
      const existingTid = await pool.query(
    ...
    await pool.query(
      "INSERT INTO trabajadores (id_trabajador, t_name, t_apellido, t_cedula, id_departamento, id_cargo) VALUES ($1, $2, $3, $4, $5, $6)",
      [tid, nombre, apellido, cedula, this.id_departamento, this.id_cargo]
    );
  ...
  static async obtainByCi(cedula) {
    const result = await pool.query(
      `SELECT t.id_trabajador, t.t_name, t.t_apellido, t.t_cedula, t.id_departamento, t.id_cargo, t.fecha_registro, 
              COALESCE(d.d_name, 'Sin asignar') AS departamento, 
              COALESCE(c.c_name, 'Sin asignar') AS cargo
       FROM trabajadores t
       LEFT JOIN departamento d ON t.id_departamento = d.id_departamento
  ...
  static async editWorker(
    ...
    const worker = await pool.query(
      "SELECT t_name, t_apellido, t_cedula, id_departamento, id_cargo FROM trabajadores WHERE t_cedula = $1",
      [cedula]
    );
    ...
    const formatText = (texto) =>
      texto
        ? texto.trim().charAt(0).toUpperCase() + texto.slice(1).toLowerCase()
    ...
    const idDepartamentoFinal =
      id_departamento || worker.rows[0].id_departamento;
    ...
    if (cedulaFinal !== cedula) {
      const exist = await pool.query(
        "SELECT id_trabajador FROM trabajadores WHERE t_cedula = $1 AND t_cedula <> $2",
        [cedulaFinal, cedula]
      );
      if (exist.rows.length > 0) {
        throw new Error(
          "La nueva cédula ya está registrada en otro trabajador"
        );
      }
    }
