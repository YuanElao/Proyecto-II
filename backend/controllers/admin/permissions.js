...
const permissionRegister = async (req, res) => {
  ...
  try {
    ...
    const tResult = await pool.query(
      "SELECT id_trabajador FROM trabajadores WHERE t_cedula = $1",
      [tCedula]
    );
    ...
    const assist = await pool.query(
      "SELECT * FROM asistencias WHERE id_trabajador = $1 AND fecha BETWEEN $2 AND $3",
      [tId, fecha_inicio, fecha_fin]
    );
    ...
    if (assist.rows.length > 0) {
      ...
      return res
        ...
        .json({
          message:
            "No se pudo registrar el permiso: hay asistencia(s) en el rango de las fechas",
        });
    }
    ...
    const faults = await pool.query(
      "SELECT * FROM faltas WHERE id_trabajador = $1 AND fecha BETWEEN $2 AND $3",
      [tId, fecha_inicio, fecha_fin]
    );
    ...
    if (faults.rows.length > 0) {
      return res
        ...
        .json({
          message:
            "No se pudo registrar el permiso: hay falta(s) en el rango de las fechas",
        });
    }
    ...
    const permission = await pool.query(
      "SELECT * FROM permisos WHERE id_trabajador = $1 AND (fecha_inicio <= $3 AND fecha_fin >= $2)",
      [tId, fecha_inicio, fecha_fin]
    );
    ...
    if (permission.rows.length > 0) {
      return res
        ...
        .json({
          message:
            "No se pudo registrar el permiso: ya existe un permiso en el rango de fechas",
        });
    }
    ...
    await pool.query(
      ...
      [tId, fecha_inicio, fecha_fin, motivo]
    );
    ...
    res
      .status(500)
      .json({ message: "Error al registar permiso", error: error.message });
  }
};
