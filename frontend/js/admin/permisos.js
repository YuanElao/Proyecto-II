document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        depSelect.addEventListener('change', async (e) => {
            ...
            cargoSelect.innerHTML = '<option value="" disabled selected>Cargando cargos...</option>';
            
            try {
                const responseCargos = await fetch(`http://localhost:3000/admin/job/list/${e.target.value}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                cargoSelect.innerHTML = '<option value="" disabled selected>Error cargando cargos</option>';
            }
        });
        ...
        cargoSelect.addEventListener('change', async (e) => {
            ...
            trabajadorSelect.innerHTML = '<option value="" disabled selected>Cargando trabajadores...</option>';
            
            try {
                const responseTrab = await fetch(`http://localhost:3000/user/consult/worker?departamento=${depSelect.value}&cargo=${e.target.value}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });

                const trabajadores = await responseTrab.json();
                trabajadorSelect.innerHTML = `<option value="" disabled selected>Trabajador</option>
                    ${trabajadores.map(t => `<option value="${t.t_cedula}">${t.nombre} ${t.apellido}</option>`).join('')}`;
                trabajadorSelect.disabled = false;

            } catch (error) {
                console.error("Error cargando trabajadores:", error);
                trabajadorSelect.innerHTML = '<option value="" disabled selected>Error cargando trabajadores</option>';
            }
        });
        ...
        document.querySelector('form').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://localhost:3000/admin/permission/add', {
                    method: 'POST',
                    headers: {
