document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function showEditModal(account, isRootAdmin) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <form class="edit-form">
                    <div class="input-box">
                        <input type="text" id="editUsername" value="${account.c_name}" 
                    ...
                    <div class="seleccionable">
                        <select id="editRole" class="seleccionable1">
                            <option value="user" ${account.c_rol === 'user' ? 'selected' : ''}>Secretario</option>
                            <option value="admin" ${account.c_rol === 'admin' ? 'selected' : ''}>Administrador</option>
                        </select>
                    </div>
                    ` : ''}
        ...
        modal.querySelector('.edit-form').addEventListener('submit', async (e) => {
            ...
            const newRole = isRootAdmin ? modal.querySelector('#editRole').value : account.c_rol;
            ...
            try {
                const endpoint = account.c_rol === 'admin' ? 
        ...
        try {
            ...
            accounts.forEach(account => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        ${canModify ? `<button class="action-btn edit-btn" data-id="${account.c_id}">Editar</button>` : ''}
                        ${canDelete ? `<button class="action-btn delete-btn delete" data-id="${account.c_id}">Eliminar</button>` : ''}
                    </td>
                `;
                
                // Asignar eventos a los botones
                if (canModify) {
                    tr.querySelector('.edit-btn').addEventListener('click', () => showEditModal(account, isRootAdmin));
                }
                if (canDelete) {
                    tr.querySelector('.delete-btn').addEventListener('click', () => handleDeleteAccount(account));
                }
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            const endpoint = role === 'admin' ? 
                'http://localhost:3000/admin/account/admin/add' : 'http://localhost:3000/admin/account/user/add';
    ...
    toggleFormBtn.addEventListener('click', () => {
        ...
        if (formContainer.classList.contains('collapsed')) {
            toggleFormBtn.innerHTML = '<i class="bx bxs-plus-circle"></i> Crear Cuenta';
        } else {
            toggleFormBtn.innerHTML = '<i class="bx bxs-minus-circle"></i> Ocultar Formulario';
        }
    });
