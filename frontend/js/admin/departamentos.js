document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderDepartamentos = (departamentos) => {
        departamentosTable.innerHTML = departamentos.map(dep => `
    ...
    const renderCargos = (cargos) => {
        cargosTable.innerHTML = cargos.map(c => `
            <tr data-id="${c.id}">
                <td>${c.nombre}</td>
    ...
    selectItem.addEventListener('change', async (e) => {
        ...
        formContainer.innerHTML = action === 'departamento' 
    ...
    const loadDepartamentosSelect = async (selector) => {
        try {
            ...
            document.querySelector(selector).innerHTML = `
                ...
                ${data.map(d => `<option value="${d.id_departamento}">${d.d_name}</option>`).join('')}
            `;
        } catch (error) {
            console.error("Error cargando departamentos:", error);
        }
    ...
    formContainer.addEventListener('click', async (e) => {
        ...
        const depId = action === 'cargo' ? document.getElementById('depSelect')?.value : null;
        ...
        try {
            const endpoint = action === 'departamento' 
            ...
            const response = await fetch(`http://localhost:3000${endpoint}`, {
                ...
                body: JSON.stringify(action === 'departamento' 
                    ? { d_name: nombre } 
    ...
    document.addEventListener('click', async (e) => {
        if (e.target.classList.contains('editar-dep')) {
            ...
            document.getElementById('editDepNombre').value = row.querySelector('td:first-child').textContent;
        ...
        if (e.target.classList.contains('editar-car')) {
            const row = e.target.closest('tr');
            currentEditId = row.dataset.id;
            document.getElementById('editCarNombre').value = row.querySelector('td:first-child').textContent;
    ...
    // Eliminar elementos
    document.addEventListener('click', async (e) => {
        if (!e.target.classList.contains('eliminar-dep') && !e.target.classList.contains('eliminar-car')) return;
    ...
    document.getElementById('btnGuardarDep').addEventListener('click', async () => {
        const nuevoNombre = document.getElementById('editDepNombre').value;
        
        try {
            const response = await fetch(`http://localhost:3000/admin/department/${currentEditId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
